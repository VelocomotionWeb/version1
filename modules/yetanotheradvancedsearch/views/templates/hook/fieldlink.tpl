       ...
       {foreach $field_values as $fieldValue}
        ...
        {if $fieldValue['count'] > 0}
            ...
            {if (!($criterion['id_criteria_type'] == CriteriaTypeEnum::FEATURE && $ignoreCustom == "true" && $fieldValue["custom"] == "1")) }
				...
				<li>
					<div data-type="link" data-allow-multiple="{$criterion['allow_multiple']|escape}" data-yaas-count="{$fieldValue['count']|escape}" data-yaas-kept-count="{$fieldValue['count']|escape}" class="yaas-criterion" data-internal-id="{$criterion['id_criteria_type']|escape}-{$fieldValue['id_criteria_field']|escape}v{$fieldValue['id_internal']|escape}">
