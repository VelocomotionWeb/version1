...
EOT;
        ...
        // Allow infos box by default
        Configuration::updateValue('pagecache_allow_infosbox', true, false, $id_shop_group, $id_shop);
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitModule'))
        {
            if (_PS_MODE_DEMO_ && !$this->context->employee->isSuperAdmin()) {
                ...
                //
                    ...
                    Configuration::updateValue('pagecache_allow_infosbox', Tools::getValue('pagecache_allow_infosbox', false));
        ...
        $html .= '<form id="pagecache_form_install" action="'.Tools::htmlentitiesutf8($_SERVER['REQUEST_URI']).'" method="post">
        ...
        //
        if ($advanced_mode) {
            $html .= '<form id="pagecache_form_options" action="'.Tools::htmlentitiesutf8($_SERVER['REQUEST_URI']).'" method="post">
            <input type="hidden" name="submitModule" value="true"/>
        ...
        // Pages and timeouts
        //
        $html .= '<form id="pagecache_form_timeouts" action="'.Tools::htmlentitiesutf8($_SERVER['REQUEST_URI']).'" method="post">
            <input type="hidden" name="submitModule" value="true"/>
        ...
        $html .= '<form id="pagecache_form_dynhooks" action="'.Tools::htmlentitiesutf8($_SERVER['REQUEST_URI']).'" method="post">
            <input type="hidden" name="submitModule" value="true"/>
        ...
        // Statistics
        //
        $html .= '<form id="pagecache_form_stats" action="'.Tools::htmlentitiesutf8($_SERVER['REQUEST_URI']).'" method="post">';
        ...
        if ($advanced_mode) {
            ...
            $html .= '<form id="pagecache_form_cachemanagement" action="'.Tools::htmlentitiesutf8($_SERVER['REQUEST_URI']).'" method="post">';
    ...
    public function hookDisplayFooter() {
        if (PageCache::canBeCached() || PageCache::isDisplayStats()) {
    ...
    public static function canBeCached() {
        ...
        $canBeCached = strcmp($_SERVER['REQUEST_METHOD'], 'GET') == 0
            && (Configuration::get('pagecache_'.$controller))
            && !self::isMaintenanceEnabled()
    ...
    /**
     * @return boolean true if user will be redirected to an other page (see FrontController::sslRedirection())
     */
    ...
    public static function cacheThis($html, $is_retro_compatible = false) {
        ...
        if (file_put_contents($cache_file, $html, $appendFlag) === false) {
    ...
    private static function _getCacheFilepath() {
        ...
        if (!file_exists($subdir)) {
            ...
            $grants = 0777;
            $cacheDirStats = stat(_PS_CACHE_DIR_);
            if ($cacheDirStats) {
                $grants = $cacheDirStats['mode'] & 000777;
            }
            mkdir($subdir, $grants, true);
        }
    ...
    public function clearCache($delete_dir = false) {
    ...
    public function hookActionFeatureSave($params) {
        if (isset($params['id_feature'])) {
            ...
            $productsIds = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
                ...
                LEFT JOIN '._DB_PREFIX_.'feature f ON (f.id_product = p.id_product)
     ...
     * @return bool
     */
    public function jmRemoveOverride($classname)
    {
        require_once 'pagecache_remove.inc';
        return call_user_func('jmRemoveOverride', $this, $classname);
    }
    ...
    public function upgradeOverride($class_name) {
        ...
        if (Tools::version_compare(_PS_VERSION_,'1.6','>=')
            ...
            $reset_ok = $this->jmRemoveOverride($class_name) && $this->addOverride($class_name);
        }
        return $reset_ok;
    }
    ...
    private static function isDisplayStats() {
        if (Tools::getIsset('ajax') || strcmp($_SERVER['REQUEST_METHOD'], 'GET') != 0) {
            return false;
        }
        return Configuration::get('pagecache_always_infosbox')
            || ((Configuration::get('pagecache_allow_infosbox') || Configuration::get('pagecache_debug')) && Tools::getIsset('dbgpagecache'));
    }
