...
class Jmsslider extends Module
{
	...
	public function createTables()
    {
        ...
        $res &= Db::getInstance()->execute('CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'jms_slides_homes` (
            `id_slide` int(10) NOT NULL,
    ...
    public function dropTables()
    {
        ...
        $res&=Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'jms_slides_homes`;');        
        return $res;
    }
    ...
    public function process()
    {
        ...
        if (Tools::isSubmit('submitSlide')) {
            ...
            $homes = $this->getHomesSlide((int)Tools::getValue('id_slide'));
            ...
            Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.'jms_slides_homes` VALUES('.$slide_dup->id.', "'.$homes[0]['id_homes'].'")');
            $this->redirectAdmin(3, '');
        }

        if (count($errs)<1) {
            $l_id_homes = Tools::getValue('home_slide');
            $str_id_homes = in_array('0', $l_id_homes)?'0':implode(',', $l_id_homes);
            if (!Tools::getValue('id_slide')) {
                ...
                $slide->add();
                DB::getInstance()->execute('INSERT INTO `'._DB_PREFIX_.'jms_slides_homes` VALUES('.$slide->id.', '.$str_id_homes.');');
                ...
                $slide->order = (int)Tools::getValue('order_slide', $slide->order);
                $slide->update();
                DB::getInstance()->execute('UPDATE `'._DB_PREFIX_.'jms_slides_homes` SET `id_homes` ="'.$str_id_homes.'" WHERE `id_slide` = '.(int)Tools::getValue('id_slide'));
    ...
    public function renderListSlide()
    {
        ...
        foreach ($slides as $slide) {
            ...
            $slides[$i]['homes'] = $this->getHomesStr($slide['id_slide']);            
            $i++;
        }
        $this->smarty->assign(
            array(
	...
	public function getJmsPageBuilderHomes() 
	{
		return DB::getInstance()->executeS('SELECT h.`id_homepage`,h.`title` FROM `'._DB_PREFIX_.'jmsadv_homepages` h ORDER BY `ordering` ASC');
	}
    public function renderFormSlide()
    {
		...
		$home_slide = array();
		$home_slide[] = array('id_homepage' => 0, 'title' => 'All Home');
		
		$homepages = array();
		$homepages = $this->getJmsPageBuilderHomes();        
    ...
    public function addFieldsSlide()
    {
        ...
        if (Tools::isSubmit('id_slide')) {
            ...
            $id_homese = $this->getHomesSlide(Tools::getValue('id_slide'));
            $list_id_homes = explode(',', $id_homese[0]['id_homes']);
            for ($i = 0; $i < sizeof($list_id_homes); $i++) {
                $fields['home_slide[]'][$i] = Tools::getValue('home_slide_'.$list_id_homes[$i], $list_id_homes[$i]);
            }
        } else {
            $slide = new SlideObject();
            $fields['home_slide[]'] = Tools::getValue('home_slide');
        }
    ...
    public function getHomesSlide($id_slide)
    {
        return DB::getInstance()->executeS('SELECT `id_homes` FROM `'._DB_PREFIX_.'jms_slides_homes`
        WHERE `id_slide` ='.$id_slide);
    }
    public function getHomesStr($id_slide)
    {
        $homes = array();
        $home_q_ids =  DB::getInstance()->getValue('SELECT `id_homes` FROM `'._DB_PREFIX_.'jms_slides_homes` WHERE `id_slide` ='.$id_slide);                
		$homes = DB::getInstance()->executeS('SELECT `title` FROM `'._DB_PREFIX_.'jmsadv_homepages` WHERE `id_homepage` IN ('.$home_q_ids.')');
		$homestr = array();
		foreach ($homes as $_home)
			$homestr[] = $_home['title'];
        return implode(',',$homestr);
    }
	...
	public function checkSlideHome($id_slide) 
	{
		...
		$homeidsstr = DB::getInstance()->getValue('SELECT `id_homes` FROM `'._DB_PREFIX_.'jms_slides_homes` `sh`             
            WHERE `sh`.`id_slide` = '.(int)$id_slide);
		if($homeidsstr == '0')	return true;
		$homeids = array();
		$homeids = explode(",",$homeidsstr);
		if(in_array($homepage_id, $homeids)) return true;
		return false;
    ...
    public function getSlidesLayers()
    {
        ...
        $temp_slides = DB::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.'jms_slides` `js` 
            LEFT JOIN `'._DB_PREFIX_.'jms_slides_lang` `jsl` ON `js`.`id_slide`=`jsl`.`id_slide`
            LEFT JOIN `'._DB_PREFIX_.'jms_slides_shop` `jss` ON `js`.`id_slide`=`jss`.`id_slide`			
		...
		$slides = array();	
		foreach($temp_slides as $_slide) 
			if($this->checkSlideHome($_slide['id_slide']))
				$slides[] = $_slide;		
        $i=0;
        foreach ($slides as $slide) {
    ...
    public function checkHomeSlide($slide, $home)
    {
        return Db::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.'jms_slides_homes`  WHERE `id_homes` = '.$home.'
                            AND `id_slide` = '.$slide);
    }
