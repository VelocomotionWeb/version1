		...
		<div class="wrapper_left_div">
            ...
            <div id="search_city_block" style="border-bottom:1px solid #dbdbdb;">
				<div id="search_city_field">
					<input  name1="ville" id="autocomplete" class="form-control" placeholder="{l s='Location' mod='blocksearchhome'}" 
                    onFocus1="geolocate()" type="text"></input>
                    <input id="search_city" class="form-control" type="text" placeholder="{l s='Enter City Name' mod='mpstorelocator'}" style="display:none" />
			...
			*}
            <div id="wrapper_content_left">
                ...
                <div id="map_full"><b>Voir la carte en grand</b></div>
                <div id="map_small"><b>Réduire</b></div>
                <div id="group-seller">	
                {if isset($store_locations)}
                    ...
                    {foreach $store_locations as $store}
                        {if $store.distance_user<=$distance}
                            ...
                            {/if}
                            <div class="wk_store see_prod" style="border-bottom:1px solid #dbdbdb" id="{$store.id|escape:'htmlall':'UTF-8'}" addr="{$store.map_address|escape:'htmlall':'UTF-8'}" lat="{$store.latitude|escape:'htmlall':'UTF-8'}" lng="{$store.longitude|escape:'htmlall':'UTF-8'}" link="{$store.link_rewrite}" counter={counter}>
                            	
                                <div class="wk_store_img">myMapOptions
                                    {if $store.img_exist}
...
$(document).ready(function(){
	...
	$("#map_full, #map_small").on("click", function() {
		if ($('#map-canvas').hasClass('map_large'))
		{
		  $('#map-canvas, #map_small').removeClass('map_large');
		}
		else
		{  
		  $('#map-canvas, #map_small').addClass('map_large');
		}
		map_reset();
	});
