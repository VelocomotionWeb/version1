...
class LemonwayConfirmationModuleFrontController extends ModuleFrontController
{
    public function postProcess()
    {
		...
		*/
		...
		Logger::AddLog("Retour de transaction sur la Commande n°". (int)$cart_id.' [ action = '.$action.'] (Weegle)');
        /**
       ...
       /* Context::getContext()->cart = $cart;
        ...
        $message = $this->module->l("Order in pending validation payment."); // You can add a comment directly into the order so the merchant will see it in the BO.
        ...
        switch ($action){
        	case 'return':
        		...
        		if(!$order_id)
        		{
	        		...
	        		$order_id = Order::getOrderByCartId((int)$cart->id);
					Logger::AddLog(" Validation de la commande ");
        		}
				Logger::AddLog(" Act 3 ");
        	...
        	case 'error':
        		...
        		{
        			$module_id = $this->module->id;
        			return Tools::redirect('index.php?controller=order-confirmation&id_cart='.$cart_id.'&id_module='.$module_id.'&id_order='.$order_id.'&key='.$secure_key);
