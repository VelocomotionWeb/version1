	...
	<div id="group-seller" class="row">
		...
		{if isset($store_locations)}
			...
			{foreach $store_locations as $key => $store}
				...
				{if $store.distance_user<=$distance && $key<10}
					...
					<div class="col-md-3 col-sm-6 col-xs-6 wk_tabs">
						...
						<form name="refresh_{$store.id}" id="refresh_{$store.id}" method="post">
							...
							<div class="wk_store {if $store.link_rewrite==$shop_link_rewrite_ec}active{/if} see_prod" id="{$store.id|escape:'htmlall':'UTF-8'}" store_name="{$store.name}" addr="{$store.map_address|escape:'htmlall':'UTF-8'}" lat="{$store.latitude|escape:'htmlall':'UTF-8'}" lng="{$store.longitude|escape:'htmlall':'UTF-8'}" link="{$store.link_rewrite}" store_postal="{$store.zip_code}" counter="{counter}">

								<div class="row">
									<div class="col-md-12 wk_store_details">

										<ul>
											<li class="store_name">{$store.name|escape:'html':'UTF-8'}</li>
											<li class="store_city_zip_code">{$store.zip_code} {$store.city_name}</li>
											{if $store.distance_user>1}
											<li class="aff_distance"><i class="fa fa-map-marker" aria-hidden="true"></i> {$store.distance_user} Km</li>
											{/if}
										...
										<input id="see_map_{$store.id|escape:'htmlall':'UTF-8'}" class="submit see_map" type="button" name="see_map" value="Voir plan" sid="{$store.id|escape:'htmlall':'UTF-8'}">

										<input id="see_prod_{$store.id|escape:'htmlall':'UTF-8'}" class="submit see_prod" type="button" name="see_prod" value="{l s='View products' mod='mpstorelocator'}" sid="{$store.id|escape:'htmlall':'UTF-8'}" link="{$store.link_rewrite}" >*}


										{*<a href="/module/marketplace/shopstore?mp_shop_name={$store.link_rewrite}" target="_blank">
											<input id="see_seller_{$store.id|escape:'htmlall':'UTF-8'}" class="submit see_seller" type="button" name="see_seller" value="{l s='Seller detail' mod='mpstorelocator'}" >
										</a>*}

									</div>


									<!-- <div class="col-md-3 wk_store_img hidden-sm hidden-xs">
										<img src="{$modules_dir|escape:'html':'UTF-8'}mpstorelocator/views/img/store_logo/logo.png" class="img-responsive"/>
									</div> -->
								</div>
