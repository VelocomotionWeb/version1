{if $MP_SHOW_SELLER_DETAILS}
	<section class="page-product-box">
        <ul class="nav nav-tabs">
            {if $product->description}<li class="active"><a href="#tabs-1" data-toggle="tab">{l s='Seller details' mod='marketplace'}</a></li>{/if}				
        </ul>
            ...
            <div class="sellerinfo">
				{if isset($mp_seller_info)}
					{if $MP_SELLER_DETAILS_ACCESS_1}
						<div class="wk_row">
							<label class="wk-person-icon">{$mp_seller_info.seller_name|escape:'html':'UTF-8'}</label>
							<span></span>
						</div>
					...
					{if $MP_SELLER_DETAILS_ACCESS_2}
						<div class="wk_row">
							<label class="wk-shop-icon">{l s='Seller' mod='marketplace'} </label>
			...
			<div class="sellerlink">
				<ul>
					{if $MP_SELLER_DETAILS_ACCESS_6}
						<li>
							<a id="profileconnect" title="{l s='View profil' mod='marketplace'}" target="_blank" href="{$link->getModuleLink('marketplace', 'sellerprofile', ['mp_shop_name' => $name_shop|escape:'html':'UTF-8'])}">{l s='View profil' mod='marketplace'}</a>
						</li>
					{/if}
					{if $MP_SELLER_DETAILS_ACCESS_7}
                    {*
						<li>
							<a id="siteconnect" title="{l s='View offered products' mod='marketplace'}" target="_blank" href="{$link->getModuleLink('marketplace', 'shopcollection',['mp_shop_name' => $name_shop|escape:'html':'UTF-8'])}">{l s='View offered products' mod='marketplace'}</a>
						</li>
                    *}
					{/if}
					{if $MP_SELLER_DETAILS_ACCESS_8}
						<li> {* &city={$city_name}&latitude={$latitude}&longitude={$longitude} *}
                    		<a href="http://www.velocomotion.com/location/{$name_shop|escape:'html':'UTF-8'}" target="_blank">
                            {l s='Recent Products' mod='marketplace'}
							</a>
						</li>
                        {*
						<li>
                            <a id="storeconnect" title="{l s='Seller detail' mod='marketplace'}" target="_blank" href="{$link->getModuleLink('marketplace', 'shopstore', ['mp_shop_name' => $name_shop|escape:'html':'UTF-8'])}">
                            {l s='Seller shop' mod='marketplace'}</a>
						</li>
                        *}
					{/if}
					{if $MP_SELLER_DETAILS_ACCESS_9}
						{*<li>
							<a href="#wk_question_form" class="open-question-form" title="{l s='Seller contact' mod='marketplace'}">{l s='Seller contact' mod='marketplace'}</a>
						</li>*}
					{/if}
