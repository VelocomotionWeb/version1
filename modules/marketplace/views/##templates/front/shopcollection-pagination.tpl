...
{if isset($p) AND $p}
			...
			<form class="showall" action="{if !is_array($requestNb)}{$requestNb|escape:'html':'UTF-8'}{else}{$requestNb.requestUrl|escape:'html':'UTF-8'}{/if}" method="get">
				<div>
	                ...
	                <button type="submit" class="btn btn-default button exclusive-medium">
	                	<span>{l s='Show all' mod='marketplace'}</span>
	                </button>
					{if is_array($requestNb)}
						...
						{/foreach}
					{/if}
	                <input name="n" id="nb_item" class="hidden" value="{$nb_products|escape:'html':'UTF-8'}" />
				</div>
			</form>
		{/if}
		{if $start!=$stop}
    ...
    <div class="product-count">
        ...
        {/if}
        {if $nb_products > 1}
        	{l s='Showing %1$d - %2$d of %3$d items' sprintf=[$productShowingStart, $productShowing, $nb_products] mod='marketplace'}
		{else}
        	{l s='Showing %1$d - %2$d of 1 item' sprintf=[$productShowingStart, $productShowing] mod='marketplace'}
       	{/if}
    </div>
	<!-- /Pagination -->
{/if}
