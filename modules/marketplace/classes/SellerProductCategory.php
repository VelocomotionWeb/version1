...
class SellerProductCategory extends ObjectModel
{
	...
	{
		$defaultcat = Db::getInstance()->getValue('SELECT `id_category` FROM `'._DB_PREFIX_.'marketplace_seller_product`
											WHERE `id` = '.$mpproductid);
		if($defaultcat)
			return $defaultcat;
	...
	{
		return Db::getInstance()->executeS("SELECT a.`id_category`, a.`id_parent`, l.`name` FROM `"._DB_PREFIX_."category` a
								LEFT JOIN `"._DB_PREFIX_."category_lang` l  ON (a.`id_category` = l.`id_category`)
								WHERE a.`id_parent` = ".$id_parent." AND l.`id_lang` = ".$id_lang."
								AND l.`id_shop` = ".Context::getContext()->shop->id."
								AND a.`active` = 1 ORDER BY a.`id_category`");
	}
