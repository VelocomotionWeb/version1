...
class etranslation extends Module {
	...
	public function __construct() {
		...
		$this->version = '2.1.1';
		$this->module_key = "8abd6d255a84ca67e81c1071d54b1e84";
	...
	public function getContent() {
		...
		elseif (Tools::isSubmit('doExport')) {
			...
			if (Tools::getValue('export_element_id') =="" || Tools::getValue('export_element_id')==null){
				$extelemid = 0;
			}else{
			...
			if (Tools::getValue('columns_to_translate') =="" || Tools::getValue('columns_to_translate')==null){
				$coltotrid = "";
			}else{
			...
			if ($email ==null || $email ==""){
				...
				}else if ( $datefirst == null && $datesecond != null)
				{
					...
					$sql = 'SELECT id_product FROM '._DB_PREFIX_.'product WHERE date_add < "'.pSQL($datesecond).'"';
					$product_rows = Db::getInstance()->ExecuteS($sql);
					...
					$sql = 'SELECT id_category FROM '._DB_PREFIX_.'category WHERE date_add < "'.pSQL($datesecond).'"';
					$categ_rows = Db::getInstance()->ExecuteS($sql); 
				...
				}else if ( $datefirst != null && $datesecond == null)
				{
					...
					$sql = 'SELECT id_product FROM '._DB_PREFIX_.'product WHERE date_add > "'.pSQL($datefirst).'"';
					$product_rows = Db::getInstance()->ExecuteS($sql);
					...
					$sql = 'SELECT id_category FROM '._DB_PREFIX_.'category WHERE date_add > "'.pSQL($datefirst).'"';
					$categ_rows = Db::getInstance()->ExecuteS($sql); 
				...
				}else if ( $datefirst != null && $datesecond != null)
				{
					...
					$sql = 'SELECT id_product FROM '._DB_PREFIX_.'product WHERE date_add BETWEEN "'.pSQL($datefirst).'" AND "'.pSQL($datesecond).'"';
					$product_rows = Db::getInstance()->ExecuteS($sql);
					
					$sql = 'SELECT id_cms FROM '._DB_PREFIX_.'cms ';
					$cms_rows = Db::getInstance()->ExecuteS($sql);
					
					$sql = 'SELECT id_category FROM '._DB_PREFIX_.'category WHERE date_add BETWEEN "'.pSQL($datefirst).'" AND "'.pSQL($datesecond).'"';
					$categ_rows = Db::getInstance()->ExecuteS($sql); 
		...
		session_start();
		$_SESSION['cookie']=$this->context->cookie;
		$_SESSION['lastActiveUpload']=time();
