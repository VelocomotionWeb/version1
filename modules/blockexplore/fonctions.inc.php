...
function MessageBR($chaine) { ?><B CLASS="Important"><? echo $chaine; ?></B><BR><? }
...
function DecomposerChemin($chemin,$action,$tri)
{
	...
	$chemindecompose = "<A HREF=./index.php?chemin=.&tri=$tri>.</A>";
	...
	if($taille > 1)
	{
		...
		for($i=2;$i<$taille;$i++)
		{
			if($chemin[$i] == "/")
			{
				...
				$chemindecompose = $chemindecompose."/<A HREF=./index.php?chemin=$cumulencode&tri=$tri>$partie</A>";
				$partie = "";
			}
...
function GetTypeAffichageFichier($extension)
{
	switch($extension)
	{
		case "htaccess" : $type = "Source";			break;
		case "css"		: $type = "Source";			break;
		...
		case "jsp"		: $type = "Source";			break;
		case "java"		: $type = "Source";			break;
	...
	switch($type)
	{
		case "Source"	   : $affichage["Type"]    = "Source";	
							 $affichage["Lien"]    = "voirfichier.php";
							 break;						 	
...
function FormatDate($time)
{
	...
	$date["nommois3L"]  = GetNomMois3L($date["nomois"]);
...
function GetDateStr($time)
{
	 ...
	 return $date["nomjour3L"]." ".$date["nojour"]." ".$date["nommois3L"]." ".$date["noannee"];
}
			...
			{		
				...
				{	
					if(mkdir("$emplacement[$i]/$fichier",0777))
					{
						...
						MessageBR("$strExplorateurRepertoire$chemin/$fichier$strExplorateurMsgDeplacer$NouvelEmplacement");
						$retouralaligne = true;
					}
					else Message("$strExplorateurErreur");
				}
