...
class JmsLayoutInstall
{
	...
	public function _addProfile($title, $profile_type, $class_suffix, $ordering = 0)
	{
		$prof = new JmsProf();
		$prof->title = $title;
					...
					foreach ($row->position as $position)
					{
						...
						foreach ($position->block as $block)
						{
							if ((string)$block['block_type'] == 'module')
								$this->_addModule($pos_id, (string)$block['module_name'], (string)$block['title'], (string)$block['hook_name'], $block_ordering, (int)$block['show_title']);
