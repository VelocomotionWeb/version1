...
class Oparteasyseoforprestashop extends Module {
    ...
    public function createMeta($str, $obj, $limit) {
        ...
        foreach ($obj AS $key => $value) 
            $str = str_ireplace('[' . $key . ']', trim($value), $str);
        
        $str = strip_tags($str);        
        $str = Tools::substr($str,0,$limit);
    ...
    public function getProducts($id_lang, $need_cat, $need_cat_parent, $selected_categories,$start = 0,$limit = 0) {
        ...
        if(count($selected_categories)>0) {
            $result = array();
            foreach($selected_categories as $id_cat) {
		$result = array_merge($result,Product::getProducts($id_lang, $start, $limit, 'id_product', 'ASC', $id_cat));
            }
	}
	else
            $result = Product::getProducts($id_lang, $start, $limit, 'id_product', 'ASC');
        ...
        foreach ($result as $product)
         ...
         foreach ($products as &$prod) {
            ...
            $prod['price_ht'] = Tools::displayPrice(Product::getPriceStatic($prod['id_product'], false, null, 6, null, false, true, 1, false, null, null, null, $specific_price_output, true, true, null, false));            
            $prod['price_ttc'] = Tools::displayPrice(Product::getPriceStatic($prod['id_product'], true, null, 6, null, false, true, 1, false, null, null, null, $specific_price_output, true, true, null, false));
