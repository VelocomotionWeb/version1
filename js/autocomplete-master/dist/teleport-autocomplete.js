...
var NO_RESULTS_TEMPLATE = '<li class="tp-ac__item no-results">Aucun r&eacute;sultat</li>';
var GEOLOCATE_TEMPLATE = '<li class="tp-ac__item geolocate">me g&eacute;olocaliser</li>';
...
var countries = "Austria,Belgium,Bulgaria,Croatia,Cyprus,Czech Republic,Denmark,Estonia,Finland,France,Germany,Greece,Hungary,Ireland,Italy,Latvia,Lithuania,Luxembourg,Malta,Netherlands,Poland,Portugal,Romania,Slovakia,Slovenia,Spain,Sweden,United Kingdom";
  ...
  _createClass(TeleportAutocomplete, [{
    ...
    value: function getCities(cb) {
      ...
      var results = _halfred2['default'].parse(JSON.parse(req.response)).embeddedArray('city:search-results').map(function (res) {
		 ...
		 //SRDEV Trier les résultats pour les pays européen
		 var sortTab = [];
		 for(var i= 0; i < results.length; i++)
		 {
			var splitTab = results[i].title.split(",");
			var country = splitTab[2].replace(" ","");
			var testp = countries.match(country);
			if(testp)
			  {
				sortTab.push(results[i]); 
			  }
		 }
		 results = sortTab;
