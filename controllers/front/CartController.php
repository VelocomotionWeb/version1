...
class CartControllerCore extends FrontController
{
    ...
    public function init()
    {
		...
		//$id_order = 35;
		//$mp_seller_info = SellerInfoDetail::getSellerInfoByProduct($id_order,"order");
		
		//var_dump($mp_seller_info);
    ...
    {
        $customization_product = Db::getInstance()->executeS('DELETE FROM `'._DB_PREFIX_.'customization`
		WHERE `id_cart` = '.(int)$this->context->cart->id.' AND `id_product` = '.(int)$this->id_product.' ');
        $customization_product = Db::getInstance()->executeS('DELETE FROM `'._DB_PREFIX_.'customization_data` WHERE id_customization NOT IN (SELECT id_customization FROM `'._DB_PREFIX_.'customization`)');
    ...
    protected function processChangeProductInCart()
    {
        $mode = (Tools::getIsset('update') && $this->id_product) ? 'update' : 'add';
		  /* Lecture de la quantité en GET */
		  $this->qty = Tools::getValue('qty');
		  //die($this->qty);
        if ($this->qty == 0) {
      ...
      if (is_array($cart_products)) {
		...
		/******SRDEV : Controle ajout location pour un même loueur********/

		//$obj_mp_seller = new SellerInfoDetail();
		/*
		$mp_seller_info = SellerInfoDetail::getSellerInfoByProduct($this->context->cart,"cart");
		$id_seller = $mp_seller_info["id"];
		
		$mp_seller_info = $obj_mp_seller->getSellerInfoByProduct($product->id,"product");
		$current_id_seller = $mp_seller_info["id"];
		
		if($id_seller!=$current_id_seller) $this->errors[] = Tools::displayError('Loueur différent!', !Tools::getValue('ajax'));
		if ($this->errors) {
...
/* AJOUTER PAR SRDEV pour ajouter la date de depart dans Customization
$this->customization_id  contient la date de depart
*/

// inserre la date de depart dans le champs de customization
$sql = "SELECT id_customization_field FROM "._DB_PREFIX_."customization_field WHERE id_product = ".$this->id_product;
$this->id_customization_field = Db::getInstance()->getValue($sql);

$sql = "INSERT INTO "._DB_PREFIX_."customization ( id_customization,id_product,id_product_attribute,id_address_delivery,id_cart,quantity,quantity_refunded,quantity_returned,in_cart)
		 VALUES (0, ".$this->id_product.", ".$this->id_product_attribute.", ".$this->id_address_delivery.", ".$this->context->cart->id.", 0, 0, 0, 1) ";
Db::getInstance()->executeS($sql);
//echo "<li>$sql";

$sql = "SELECT max(id_customization) max FROM "._DB_PREFIX_."customization";
$result = Db::getInstance()->executeS($sql);
$this->customization_id = $result[0][max];
//echo "<li>$sql";
	
$sql = "INSERT INTO "._DB_PREFIX_."customized_data VALUES (".$this->customization_id.", 1, ".$this->id_customization_field.", '".Tools::getValue('date_depart')."') ";
Db::getInstance()->executeS($sql);	
//echo "<li>$sql " . $this->customization_id;
