								...
								<tr class="cart_total_price">
									<td rowspan="{3+$total_discounts_num+$use_show_taxes+$total_wrapping_taxes_num}" colspan="2" id="cart_voucher" class="cart_voucher">
										<!-- {if $voucherAllowed}
											...
											{if $displayVouchers}
												<h3 id="title" class="title-offers">{l s='Take advantage of our exclusive offers:'}</h3>
												<div id="display_cart_vouchers">

												{foreach $displayVouchers as $voucher}
													{if $voucher.code != ''}
										                                                    {/if}{$voucher.name} <br>{l s='Click on code'} :
							...
							<td colspan="2" class="text-right">
								{if $display_tax_label}
									{if $use_taxes && $priceDisplay == 0}
										{l s='Total vouchers'} {* TTC *}
									{else}
										{l s='Total vouchers'} {* HT *}
									{/if}
								{else}
									{l s='Total vouchers'}
								{/if}
								...
								{foreach $customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] as $id_customization=>$customization}
									<tr
										...
										<td colspan="5" style="width:100%">
											...
											{else}
                                                {l s='For a periode of' mod='mpstorelocator'} {$customization.quantity} {l s='day' mod='mpstorelocator'}{if $customization.quantity>1}s{/if}
												<input type="hidden" value="{$customization.quantity}" class="cart_quantity_input form-control grey" name="quantity_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}_{$product.id_address_delivery|intval}"/>
						...
						<tbody>
							{foreach $discounts as $discount}
								<tr class="cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
									...
									{*
                                    <td1 class="cart_discount_price">
										<span class="price-discount">
										{if !$priceDisplay}{displayPrice price=$discount.value_real*-1}{else}{displayPrice price=$discount.value_tax_exc*-1}{/if}
										</span>
									</td>
									<td1 class="cart_discount_delete">1</td>
									*}
                                    <td colspan=2 class="cart_discount_price">
                ...
                {if $delivery->id}
					<div class="col-xs-12 col-sm-6"{if !$have_non_virtual_products} style="display: none;"{/if}>
						<ul id="delivery_address" class="address item address-box">
							<li class="address_title"><h4 class="page-subheading">{l s='Delivery address'}&nbsp;<span class="address_alias">({$delivery->alias})</span></h4></li>
				...
				{if $invoice->id}
					<div class="col-xs-12 col-sm-6">
						<ul id="invoice_address" class="address alternate_item address-box">
							<li class="address_title"><h4 class="page-subheading">{l s='Invoice address'}&nbsp;<span class="address_alias">({$invoice->alias})</span></h4></li>
