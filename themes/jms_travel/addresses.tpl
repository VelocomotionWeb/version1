    	...
    	<div class="address col-xs-12 col-sm-6">
			<ul class="{if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if}{if $smarty.foreach.myLoop.index % 2} alternate_item{else} item{/if} box">
                ...
                {foreach from=$address.ordered name=adr_loop item=pattern}
                    {if ($pattern=="firstname lastname"||$pattern=="Country:name"||$pattern=="phone") }
                ...
                <a class="button" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}")|escape:'html':'UTF-8'}" title="{l s='Update'}"><span><i class="fa fa-pencil"></i>{l s='Update'}</span></a>
                <a class="button" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}&delete")|escape:'html':'UTF-8'}" onclick="return confirm('{l s='Are you sure?' js=1}');" title="{l s='Delete'}"><span><i class="fa fa-times"></i>{l s='Delete'}</span></a></li>
            </ul>
        </div>
