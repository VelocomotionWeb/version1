...
{else}
	...
	<div class="block_various_links" id="block_various_links_footer">
		...
		<div>
			<ul class="menu">
				{if !$PS_CATALOG_MODE}<li><a href="{$link->getPageLink('best-sales')|escape:'html'}" title="{l s='Best sellers' mod='blockcms'}">{l s='Best sellers' mod='blockcms'}</a></li>{/if}
				<li><a href="{$link->getPageLink('new-products')|escape:'html'}" title="{l s='New Products' mod='blockcms'}">{l s='New Products' mod='blockcms'}</a></li>
				<li><a href="{$link->getPageLink($contact_url, true)|escape:'html'}" title="{l s='Contact us' mod='blockcms'}">{l s='Contact us' mod='blockcms'}</a></li>
				{foreach from=$cmslinks item=cmslink}
					{if $cmslink.meta_title != ''}
						<li><a href="{$cmslink.link|addslashes|escape:'html'}" title="{$cmslink.meta_title|escape:'html':'UTF-8'}">{$cmslink.meta_title|escape:'html':'UTF-8'}</a></li>
					{/if}
				{/foreach}								
			</ul>
		</div>
