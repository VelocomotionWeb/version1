		...
		<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
			...
			<td class="discount_cumulative">
				{if $discountDetail.cumulable == 1}
					<i class="fa fa-check-circle"> {l s='Yes'}
				{else}
					<i class="icon_trash_alt"> {l s='No'}
				{/if}
			</td>
...
{else}
	<div class="alert alert-warning"><button data-dismiss="alert" type="button" class="close">X</button>{l s='You do not have any vouchers.'}</div>
{/if}
