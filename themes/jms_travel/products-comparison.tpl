	...
	<div class="products_block table-responsive">
		<table id="product_comparison" class="table table-bordered">
			<tr>
				...
				<td class="ajax_block_product comparison_infos product-block product-{$product->id}" itemscope itemtype="http://schema.org/Product">
					<div class="remove">
						<a 
						class="cmp_remove" 
					...
					<div class="product-preview">
						<div class="preview">
							<a class="preview-image product_img_link" 
							href="{$product->getLink()|escape:'html':'UTF-8'}" itemprop="url">
								<img class="img-responsive" src="{$link->getImageLink($product->link_rewrite, $product->id_image, 'home_default')|escape:'html':'UTF-8'}" 
								alt="{$product->name|escape:'html':'UTF-8'}" itemprop="image"/>
							</a>
							<div class="preview-button">								
								<a rel="{$product->getLink()|escape:'html':'UTF-8'}" class="quick-view product-btn hidden-xs" title="{l s='Quick View'}">
						...
						<div class="product-info">
							<h3 class="title pull-left" itemprop="name">
								<a href="{$product->getLink()|escape:'html':'UTF-8'}">{$product->name|truncate:25:'...'|escape:'html':'UTF-8'}</a>
							...
							<div class="content_price pull-right" itemprop="offers" itemscope itemtype="http://schema.org/Offer">		
								{if isset($product->show_price) && $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}										
									{if isset($product->specificPrice) && $product->specificPrice}
										...
										{/if}
									{/if}
									<span class="price new" itemprop="price">{convertPrice price=$product->getPrice($taxes_behavior)}</span>
									{if $product->on_sale}
							...
							<div class="products-box">
								{if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
									{if ($product->quantity > 0 OR $product->allow_oosp)}
										<a
											class="hvr-sweep-to-right ajax_add_to_cart_button product-btn cart-button"
											data-id-product="{$product->id}"
									...
									{else}
										<a href="#" class="disable product-btn cart-button">
											<span>{l s='Add To Cart'}</span>
										</a>
									{/if}
								{/if}
